...
function getParentCategory(){
	...
	if(json.rtState){
		...
		for(var i = 0 ;i<data.length;i++){
			var html="<option value='"+data[i].sid+"'>"+data[i].categoryName+"</option>";
			if(parentId==data[i].sid){
				 html="<option value='"+data[i].sid+"' selected='selected'>"+data[i].categoryName+"</option>";
			}
...
<body style="overflow: hidden; font-size: 12px; padding: 20px 0;"
	...
	<form id="form1" name="form1" method="post">
		<table class='TableBlock' style='width: 90%; margin: 0 auto;'>
			<tr>
				<td class='TableData'>父级分类：</td>
				<td class='TableData'><select id="parentId" name="parentId"
					class="BigSelect">
